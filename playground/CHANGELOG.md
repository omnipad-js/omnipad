# playground

## 0.2.0-alpha.3

### Patch Changes

- Perf: Displacement Optimization
- Updated dependencies
  - @omnipad/core@0.2.0-alpha.3
  - @omnipad/vue@0.2.0-alpha.3

## 0.2.0-alpha.2

### Minor Changes

- feat: implement Trackpad with gesture support, optimized focus handling.

### Patch Changes

- Updated dependencies
  - @omnipad/core@0.2.0-alpha.2
  - @omnipad/vue@0.2.0-alpha.2

## 0.1.1-alpha.1

### Patch Changes

- Fixed multiple Registry issue.
- Updated dependencies
  - @omnipad/core@0.1.1-alpha.1
  - @omnipad/vue@0.1.1-alpha.1

## 0.1.1-alpha.0

### Patch Changes

- initial architectural preview with keyboard support
- Updated dependencies
  - @omnipad/core@0.1.1-alpha.0
  - @omnipad/vue@0.1.1-alpha.0

## 0.1.0

### Minor Changes

- Initial release of OmniPad architecture.

### Patch Changes

- Updated dependencies
  - @omnipad/core@0.1.0
  - @omnipad/vue@0.1.0
